...
def generate_farm_id(name: str, place_id: str) -> str:
    ...
    # Deterministic hash from name + Google place_id
    h = hashlib.sha1(f"{name}-{place_id}".encode()).hexdigest()
    return f"farm_{h[:10]}"
...
def parse_address_components(address_components: List[Dict], formatted_address: str) -> Dict[str, str]:
    ...
    # UK County mapping for common administrative areas
    uk_county_mapping = {
        # England counties
    ...
    for component in address_components:
        ...
        if "street_number" in types or "route" in types:
            ...
            # This could be a country or region, but let's check if it's a known county
            if long_name in uk_county_mapping:
                result["county"] = uk_county_mapping[long_name]
            elif long_name in ["England", "Scotland", "Wales", "Northern Ireland"]:
                # This is a country/region, not a county
                result["country"] = long_name
            else:
                # Try to map it to a county
                result["county"] = uk_county_mapping.get(long_name, long_name)
        elif "postal_code" in types:
            result["postcode"] = long_name
        elif "country" in types:
            result["country"] = long_name
    
    # If we still don't have a county, try to extract from formatted address
    if not result["county"] and formatted_address:
        # Look for common county patterns in the address
        address_lower = formatted_address.lower()
        for county_name in uk_county_mapping.keys():
            if county_name.lower() in address_lower:
...
async def main():
    ...
    with open(output_dir / "farms.uk.json", "w", encoding="utf-8") as f:
        json.dump([shop.model_dump() for shop in shops], f, indent=2, ensure_ascii=False)
