...
GOOGLE_API_KEY = os.getenv("GOOGLE_MAPS_API_KEY")  # Required for Google Geocoding API
...
async def geocode_missing(shops: List[FarmShop]) -> None:
    if not GOOGLE_API_KEY:
        print("⚠️  GOOGLE_MAPS_API_KEY not set - skipping geocoding", file=sys.stderr)
        return
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for s in shops:
            if s.location.lat is not None and s.location.lng is not None:
                continue
            address = " ".join([s.location.address, s.location.postcode or "", "United Kingdom"]).strip()
            params = {
                "address": address,
                "key": GOOGLE_API_KEY,
            ...
            try:
                ...
                if data.get("status") == "OK" and data.get("results"):
                    ...
                    s.location.lat = location["lat"]
                    s.location.lng = location["lng"]
                    print(f"✅ Geocoded {s.name}: {s.location.lat}, {s.location.lng}")
                else:
                    print(f"❌ Geocoding failed for {s.name}: {data.get('status')}", file=sys.stderr)
            except Exception as e:
                print(f"❌ Geocoding error for {s.name}: {e}", file=sys.stderr)
            await asyncio.sleep(0.1)  # Google allows 10 requests per second
