...
async function checkOrphanedPhotos() {
  ...
  // Find photos with farmSlug that doesn't exist in farms table
  const orphanedPhotos = await prisma.$queryRaw<Array<{id: string, farmSlug: string, status: string}>>`
    SELECT p.id, p."farmSlug", p.status
    FROM "Image" p
...
async function checkPhotoStatus() {
  ...
  const invalidPhotoStatus = await prisma.$queryRaw<Array<{id: string, farmSlug: string, status: string}>>`
    SELECT id, "farmSlug", status
    FROM "Image"
  ...
  if (invalidPhotoStatus.length > 0) {
    for (const photo of invalidPhotoStatus) {
      logIssue({
        ...
        message: `Photo for farm "${photo.farmSlug}" has invalid status: ${photo.status}`,
        recordId: photo.id,
        details: { farmSlug: photo.farmSlug, status: photo.status }
      })
    }
  } else {
