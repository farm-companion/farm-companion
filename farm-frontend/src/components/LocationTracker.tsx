...
export default function LocationTracker({ 
  ...
  // Full desktop and mobile version
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
      {/* Mobile: Large Primary Location Button */}
      <div className="md:hidden">
        <button
          onClick={onZoomToLocation || getCurrentLocation}
          disabled={isLoading || permission === 'denied'}
          className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl hover:from-blue-600 hover:to-blue-700 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg text-lg font-medium min-h-[56px]"
        >
        ...
        {/* Mobile: Secondary Track Button */}
        <div className="mt-3 flex gap-2">
          {!isTracking ? (
            <button
              onClick={startTracking}
              disabled={permission === 'denied'}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
      ...
      {/* Mobile: Location Status (Compact) */}
      <div className="md:hidden">
        {userLocation && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-blue-600" />
                <span className="text-sm font-medium text-blue-800">Located</span>
                {userLocation.isTracking && (
                  <div className="flex items-center gap-1">
                    <Wifi className="w-3 h-3 text-green-600" />
                    <span className="text-xs text-green-600">Live</span>
