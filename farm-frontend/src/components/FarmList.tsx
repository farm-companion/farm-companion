...
export default function FarmList({
      ...
      >
        ...
        <div className="flex items-start justify-between mb-2">
          ...
          <div className="flex flex-col items-end gap-1">
            {/* Opening Status */}
            {openingStatus && (
              <div className={`flex items-center gap-1 text-captionall px-2 py-1 rounded-full ${
            ...
            {/* Verified Badge */}
            {farm.verified && (
              <div className="flex items-center gap-1 text-captionall text-green-600 bg-green-50 px-2 py-1 rounded-full">
            ...
            {/* Distance Badge */}
            {hasDistance && (
              <div className="flex items-center gap-1 text-captionall text-serum bg-serum/10 px-2 py-1 rounded-full">
          ...
          {farm.offerings && farm.offerings.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {farm.offerings.slice(0, 3).map((offering, idx) => (
                <span
                  key={idx}
                  className="inline-block px-2 py-1 bg-serum/10 text-serum text-captionall rounded-full"
                >
                  {offering}
                </span>
              ))}
              {farm.offerings.length > 3 && (
                <span className="inline-block px-2 py-1 bg-gray-100 text-gray-600 text-captionall rounded-full">
        ...
        {hasDistance && (
          <div className="mt-2 text-captionall text-gray-500 flex items-center gap-1">
            <Navigation className="w-3 h-3" />
  ...
  const EmptyState = useCallback(() => (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <h3 className="text-heading font-medium text-gray-900 dark:text-white mb-2">No farms found</h3>
