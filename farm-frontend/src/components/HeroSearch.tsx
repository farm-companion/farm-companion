    ...
    <div className={`w-full max-w-3xl mx-auto ${className}`}>
      <form onSubmit={handleSearch} className="relative">
        ...
        <div className="relative flex items-center bg-white dark:bg-gray-900 rounded-2xl shadow-premium-xl overflow-hidden border border-gray-200 dark:border-gray-800 transition-all duration-fast hover:shadow-premium-xl focus-within:ring-2 focus-within:ring-brand-primary">
          ...
          <input
            ...
            className="flex-1 py-6 pl-16 pr-4 text-heading bg-transparent border-none outline-none placeholder-gray-400 dark:text-white"
          />
          ...
          <Button
            type="submit"
            variant="primary"
            className="m-2 px-8 py-4 text-heading font-semibold"
          >
            Search
          </Button>
        </div>
        ...
        {showFilters && (
          <div className="mt-4 p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-premium border border-gray-200 dark:border-gray-800 animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-heading font-semibold text-gray-900 dark:text-white">Filters</h3>
              <button
                type="button"
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {filters.location && (
                <button
                  ...
                  className="px-4 py-2 rounded-full text-caption font-medium bg-brand-primary text-white hover:bg-brand-primary/90 transition-colors flex items-center gap-2"
                >
                  Near me
                  <X className="w-4 h-4" />
                </button>
      ...
      {!query && !showFilters && (
        <div className="mt-6 animate-fade-in">
          <p className="text-caption text-gray-600 dark:text-gray-400 mb-3">Popular searches:</p>
          <div className="flex flex-wrap gap-2">
            {POPULAR_SEARCHES.map((search) => (
              <button
                ...
                className="px-3 py-1.5 text-caption bg-white/80 dark:bg-gray-800/80 text-gray-700 dark:text-gray-300 rounded-full hover:bg-white dark:hover:bg-gray-800 hover:shadow-premium transition-all duration-fast backdrop-blur-sm"
              >
                {search}
              </button>
            ))}
