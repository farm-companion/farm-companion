...
export default async function ProducePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-8">
      ...
      {/* HERO */}
      <section className="rounded-[2px] overflow-hidden relative border border-border">
        <div className="relative aspect-[4/3] sm:aspect-[3/2] min-h-[280px] sm:min-h-[360px]">
          {heroImage ? (
            ...
            <div className="w-full h-full bg-surface border border-border flex items-center justify-center">
              <div className="text-center">
                <div className="w-16 h-16 bg-paper rounded-full mx-auto mb-4 flex items-center justify-center border border-border">
                  <Sprout className="w-8 h-8 text-brand" />
                </div>
                <p className="text-ink-muted font-medium">{p.name}</p>
              </div>
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-5 right-5 flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <span className={`px-3 py-1 rounded-full text-caption font-semibold ${inSeason ? 'bg-accent text-accent-text' : 'bg-surface-2 text-ink-muted'}`}>
                {inSeason ? 'In Season Now' : 'Out of Season'}
              </span>
              {isPeak && (
                <span className="px-3 py-1 rounded-full text-caption font-semibold bg-brand text-brand-text">
                  Best This Month
                </span>
              )}
            </div>
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-3">
        <Link
          ...
          className="flex items-center justify-center gap-2 rounded-[2px] border border-border bg-surface py-3 hover:bg-surface-2 transition-colors motion-reduce:transition-none"
        >
      ...
      <section className="mt-8">
        <h2 className="text-heading font-semibold text-ink">Seasonality</h2>
          ...
          {Array.from({ length: 12 }, (_, i) => {
            ...
            return (
              <div key={month} className="text-center">
                <div
                  className={[
                    'h-8 rounded-[2px] border text-small flex items-center justify-center',
                    active ? 'bg-accent border-accent text-accent-text' : 'bg-surface-2 border-border text-ink-muted',
                    peak ? 'ring-2 ring-brand' : '',
                    now ? 'outline outline-2 outline-offset-2 outline-brand' : '',
                  ].join(' ')}
      ...
      {/* GALLERY */}
      <section className="mt-8">
        <h2 className="text-xl font-semibold mb-4 text-ink">Gallery</h2>
        <ClientProduceImages
          produceSlug={p.slug}
          produceName={p.name}
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-3">
        {p.selectionTips?.length ? (
          <div className="rounded-[2px] border border-border bg-surface p-4">
            <h3 className="font-semibold text-ink">How to Choose</h3>
            <ul className="mt-2 list-disc pl-5 space-y-1 text-caption text-ink-muted">
        ...
        {p.storageTips?.length ? (
          <div className="rounded-[2px] border border-border bg-surface p-4">
            <h3 className="font-semibold text-ink">Storage Tips</h3>
            <ul className="mt-2 list-disc pl-5 space-y-1 text-caption text-ink-muted">
        ...
        {p.prepIdeas?.length ? (
          <div className="rounded-[2px] border border-border bg-surface p-4">
            <h3 className="font-semibold text-ink">Prep & Use</h3>
            <ul className="mt-2 list-disc pl-5 space-y-1 text-caption text-ink-muted">
      ...
      {p.nutritionPer100g && (
        <section className="mt-8">
          <h2 className="text-heading font-semibold text-ink">Nutrition (per 100g)</h2>
          <div className="mt-3 overflow-x-auto">
            <table className="min-w-[420px] w-full text-caption border border-border rounded-[2px] overflow-hidden bg-surface">
              <tbody>
                {Object.entries(p.nutritionPer100g).map(([k,v]) => (
                  <tr key={k} className="border-b border-border last:border-b-0">
                    <td className="px-3 py-2 font-medium capitalize bg-surface-2 text-ink">{k.replace(/([A-Z])/g,' $1')}</td>
                    <td className="px-3 py-2 text-ink-muted">{v}{k==='kcal' ? '' : k==='protein'||k==='carbs'||k==='fiber'||k==='fat'||k==='sugars' ? ' g' : ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {p.recipeChips && p.recipeChips.length > 0 && (
        <section className="mt-8">
          <h2 className="text-xl font-semibold mb-4 text-ink">Recipe Inspiration</h2>
          <div className="grid gap-3 sm:grid-cols-3">
            {p.recipeChips.map((recipe, index) => (
              <a
                ...
                className="block rounded-[2px] border border-border bg-surface p-4 hover:border-brand transition-colors motion-reduce:transition-none group"
              >
                <h3 className="font-semibold text-ink group-hover:text-brand transition-colors mb-2">
                  {recipe.title}
                </h3>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-small text-ink-muted">External recipe</span>
                  <ExternalLink className="w-4 h-4 text-ink-muted group-hover:text-brand transition-colors" />
                </div>
              </a>
            ))}
          </div>
          <div className="mt-4 p-3 bg-surface-2 rounded-[2px] border border-border">
      ...
      <section className="mt-10">
        <h2 className="text-xl font-semibold mb-4 text-ink">Related Produce</h2>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-small text-ink-muted">View all produce</span>
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-small text-ink-muted">Explore map</span>
              <ArrowRight className="w-4 h-4 text-ink-muted group-hover:text-brand transition-colors" />
            </div>
          </Link>
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="text-small text-ink-muted">View directory</span>
              <ArrowRight className="w-4 h-4 text-ink-muted group-hover:text-brand transition-colors" />
            </div>
          </Link>
        </div>
      ...
      <section className="mt-10">
        <Link
          href={`/map?q=${encodeURIComponent(p.name)}`}
          className="inline-flex items-center gap-2 rounded-[2px] bg-brand text-brand-text px-5 py-3 font-semibold hover:bg-brand-hover transition-colors motion-reduce:transition-none"
        >
